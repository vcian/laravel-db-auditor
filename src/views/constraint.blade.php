 <div class="mx-2 my-1">
     <div class="space-x-1">
        TABLE NAME : <span class="px-1 bg-blue-500 text-white">{{ str_replace("_", ' ', strtoupper($data['table'])) }}</span>
     </div>
         ...
         @foreach ($data['constrain'] as $key => $value)
             @if ($value)
                 ...
                 @foreach ($value as $constrainField)
                     @if ($key === 'foreign')
                         <div class="flex space-x-1">
                             <span class="font-bold">{{ $constrainField['colum_name'] }}</span>
                             <span class="flex-1 content-repeat-[.] text-gray"></span>
                             <i class="text-blue">{{ $constrainField['foreign_table_name'] }}</i>
                             <span class="font-bold text-green">{{ $constrainField['foreign_colum_name'] }}</span>
                         </div>
                     @else
                         <div class="flex space-x-1">
